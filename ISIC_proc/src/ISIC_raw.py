...
def load_data(image_folder, metadata_file):
    ...
    for i, row in metadata.iterrows():
        ...
        if os.path.exists(image_path):
            # Process each image and store the flattened image data
            flattened_image = load_and_process_image(image_path)
...
def prepare_dataset(archive_file, metadata_path, force=False):
    ...
    DATA_DIR = os.path.join(path, "../data/raw_datasets")
    ...
    os.system(f"cp {metadata_file} {raw_datasets_folder}")

    print("Metadata file copied successfully.")

    return image_folder, metadata_file, raw_datasets_folder
...
def save(x_train, x_test, metadata_train, metadata_test, raw_datasets_folder):
    # Save the datasets with metadata
    save_datasets(
        (x_train, metadata_train), os.path.join(raw_datasets_folder, "train_data.pkl")
    )
    save_datasets(
        (x_test, metadata_test), os.path.join(raw_datasets_folder, "test_data.pkl")
    )
...
if __name__ == "__main__":
    ...
    image_folder, metadata_file, raw_datasets_folder = prepare_dataset(
        archive_file, meta
    )
    ...
    save(x_train, x_test, metadata_train, metadata_test, raw_datasets_folder)
    print("Datasets saved successfully.")
    print("Done.")
